...
### 探测线用低彩度的颜色,不再用虚线区分 **[用户]**

灰阶配色下,四条探测靠 `strokeDasharray` 区分。**这在桶宽几分钟时管用,在每一次 ping 都上图之后就不管用了**——
一分钟一个样本,虚线的周期比抖动本身还短,点线和虚线画出来都只是"纹理"。而这是整页唯一一处多条序列共用一根轴的地方,
其它面板都是一个面板一条线,继续保持灰阶。

颜色要**暗淡**:彩度 0.055–0.085,大约是常规图表配色的三分之一;色相取各自最灰的那一端——
石板蓝 / 赭陶 / 鼠尾草 / 梅紫 / 赭黄。第四条试过 320,出来是糖果品红,换成 305。

两件事是量出来的不是看出来的:

1. **对底色的对比度**最低 3.45:1(浅色)/ 3.71:1(暗色),图形元素的要求是 3:1。
2. **去掉虚线之后颜色是唯一的区分通道**,所以明度要拉开——赭陶 vs 鼠尾草、石板蓝 vs 梅紫,正是红绿色盲会合并的两对。
   拉开后这两对彼此的灰度对比是 1.25:1,拉平时是 1.03:1(等于没有)。**达不到能安全区分的 1.4:1**:
   四种这么暗淡的色相在中性底色上,做不到又安静又离得远,而调亮和加回虚线都被否了。
   兜底是图例——点一下就能把某条线关掉。

**暗色的明度梯是浅色那条整段下移,不是向上拉伸。** 为了够到近黑底而往上抬,赭陶和梅紫会变成粉彩;
而区分两条线靠的是它们**彼此之间**的比值,整段平移不影响——#cb8e72 和 #e6a68a 是同样的 1.34:1。
