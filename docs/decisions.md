...
- 于是 1/6/24 小时都是全量,**7 天才发生聚合——而那正是中位数和带开始有意义的地方**,所以延迟 tab 也放开了 7 天
...
| 负载 | 和 CPU 共用那根标着 `%` 的轴 | 负载 0.34 画在 0.34% 处;CCS 三核上 4.32 的真实排队画在 5% 网格线以下 | 独立右轴,下限 = 核数,所以「每核 1.0 = 正好忙满」永远在轴上 |
