...
if boot_id 变了:
    ...
    delta = max(当前读数 - 上次读数, 0)   ← 读数变小只重新对基线，这一轮不计
...
**读数变小但 `boot_id` 没变，绝不能把当前读数当增量。** 三种原因会让读数变小：网卡驱动重载（计数器真归零）、
32 位计数器回绕、以及**被计入的接口消失**（`wg0` 关掉、隧道停了——`SKIP_IFACES` 挡不住它们）。
前两种下当前读数确实是新流量，第三种下它是**剩余接口的历史累计**，整个计进去等于把这台机器的历史再记一遍。

从数据上分不出是哪一种，但赌错的代价差几个数量级：接口消失时虚增可达 TB（跑一个月的机器就是几十 GB），
而重新对基线最多丢一个上报间隔的量（1 秒，几 MB）。所以选后者。

顺带一提，32 位回绕这条今天已经不成立了——`/proc/net/dev` 在 64 位内核上是 `u64`，实测节点的
