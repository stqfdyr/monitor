...
### 探测序列有自己的采样预算,比指标粗 **[默认]**

`PROBE_POINTS = 250`,不跟视口走。理由:**这是统计问题不是像素问题**——桶里得有够多的答复,
中位数才有中间、带才有两端。24 小时窗口下这是 300 秒一个桶、288 个点、每桶 5 个样本,
正好是 Smokeping 日图的形状。1 小时和 6 小时仍然一桶一个样本,也就是每一次 ping 都在图上,
本来就不需要带。

手机和宽屏拿到同一张延迟图,这是对的——聚合深度不该跟屏幕宽度走。副作用是载荷反而小了:
24 小时的探测行 2 868 → 1 100,明文 236 KB → 165 KB。
