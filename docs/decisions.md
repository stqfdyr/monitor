...
完整机制见 [traffic.md](traffic.md)。核心：agent 报内核 lifetime 计数器 + `boot_id`，hub 检测到 `boot_id` 变化就把当前读数整个计为增量。

**否决**：读数变小也照重启处理（原本就是这么写的）。读数变小还有一种原因是被计入的接口消失了——`wg0` 关掉、
隧道停了——这时当前读数是剩余接口的历史累计，整个计进去就把这台机器的历史又记了一遍，跑一个月的机器一次虚增
几十 GB。分不出是哪种原因，但重新对基线最多丢一个上报间隔，赌错的代价小几个数量级。
