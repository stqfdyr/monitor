...
### 探测的桶固定 5 分钟,不是一个点数 **[默认]**

**这是统计问题不是像素问题**:桶里得有够多的答复,中位数才有中间、带才有两端。所以探测的桶宽是
写死的 300 秒——Smokeping 的 ping 周期,也是它三十年来画图用的桶宽——而不是又一个点数预算。
后果是**手机和宽屏拿到同一张延迟图**,以及 6 小时和 24 小时是同一张图的两个 zoom,不是两种风格。

例外是窗口太短花不起:1 小时按 5 分钟分桶只有 12 个点,那不叫形状。低于 60 点就退回指标的
step,1 小时因此是每一次 ping 都在图上,本来也不需要带。
...
**第一版写错了**,写成 `PROBE_POINTS = 250` 的点数预算。整除向下取整让 `360/250 = 1`,预算在
6 小时窗口下完全没生效,结果 **6 小时是 360 点、比 24 小时的 288 点还密**——窗口越窄反而越难读。
副作用是载荷反而小了:24 小时的探测行 2 868 → 1 100,明文 236 KB → 165 KB。

### `sample_step` 向上取整,否则「预算」不是预算 **[默认]**

原来是 `60 * (hours * 60 / budget).max(1)`。整除向下取整意味着**窗口除不尽时保留更细的 step,
点数就超上限**:`hours=23` 返回 1380 点/序列,对着 720 的上限超了 1.9 倍。
...
step 让预算被突破时它照样绿。现在直接断言 721,并且把除不尽的 13 和 23 小时加进了用例。

### 图表请求带上「这块屏能画几个点」 **[默认]**

`/api/nodes/:id/metrics?points=N`，学 Grafana 面板把自己的宽度发给后端。原来 `SAMPLES` 写死 720，
手机拿到 720 个点铺进 280px 宽的图里——**2.5 个采样一个像素**，纯混叠，还为此走了 232 kB 移动流量。
现在同一个请求是 157 kB。

**只往下调，不往上调**：上限还是 hub 的 720。这条路径不要凭证，谁都能请求，预算的天花板不能让调用方定。
